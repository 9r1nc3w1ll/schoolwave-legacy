...
import { FeeTemplateFormValues, FeeTemplateInterface, IClientError, RefinedFeeItem, SessionStatus, UserSession } from "@/types";
import { SetStateAction, useEffect, useState } from "react";
import { editFeeTemplate, getFeeItems } from "@/apicalls/fees";
...
const EditFeeTemplate = (props: EditFeeTemplateProps) => {
  ...
  const { data, refetch } = useQuery("feeitems", () => getFeeItems(props.user_session?.access_token as string), { enabled: false });
  ...
  return (
    <div className="">
      <form className="space-y-5" onSubmit={onSubmit}>
        ...
        <div>
          <ClassSelect register={register} user_session={props.user_session} triggerFetch= {props.user_session_status === "authenticated"} class_selector="class_id" />
        </div>
        <div>
          <DiscountSelect register={register} trigger={props.user_session_status === "authenticated"} user_session={props.user_session as UserSession} />
        </div>
        <div>
