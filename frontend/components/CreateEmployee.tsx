...
import { CreateStaff, getStaffRoles } from "@/apicalls/staffs";
import { IClientError, RefinedFeeItem, SessionStatus, UserSession } from "@/types";
...
const CreateEmployee = (props: CreateEmployeePropsType) => {
  ...
  const { register, handleSubmit } = useForm<CreateEmployeeFormValues>({ shouldUseNativeValidation: true });
  const { data, refetch } = useQuery("staffRole", () => getStaffRoles(props?.userSession?.access_token as string), { enabled: false });
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    mutate({
      ...
      data: { ...transData, roles: selectedRoles, school: props.userSession.school.id },
    });
  });
