...
import { createExamQuestion } from '@/api-calls/exam';
...
const CreateExamQuestions = (props: any) => {
  ...
  const {
    ...
    status: status2,
    refetch: refetch2,
  } = useQuery('terms', () => getSubjects(user_session?.access_token), {
    enabled: false,
  });
  ...
  return (
    <div className='panel flex-1 px-3 py-6 ltr:xl:mr-6 rtl:xl:ml-6'>
      <div className='mt-0 w-full border-b-2 pt-0'>
        <div className='pl-3 text-base font-bold'>Create Exam Question</div>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className='px-2 py-4'>
        <div className='py-6 text-xl font-bold'>Question Details</div>
