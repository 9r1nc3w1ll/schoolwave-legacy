...
import { editExam, getExamsQuestions } from '@/api-calls/exam';
...
const EditExamForm = (props: any) => {
  ...
  const { mutate, isLoading, error } = useMutation(
    (data) =>
      editExam(props.sessionData.id, props.user_session.access_token, data),
    {
  ...
  const {
    ...
    status: status3,
    refetch: refetch3,
  } = useQuery('terms', () => getSubjects(props.user_session?.access_token), {
    enabled: false,
  });
  const {
    data: examsQuestions,
    ...
    'examquestionss',
    () => getExamsQuestions(props.user_session?.access_token),
    { enabled: false }
  );
  ...
  const [examquestionsOptions, setexamquestionsOptions] = useState<
    examsQuestionOption[]
  >([]);
  ...
  question_options = examquestionsOptions?.map(
