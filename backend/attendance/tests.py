...
#         url = reverse("student_class_attendance_retrieve", kwargs={"pk":self.attendance.class_id.id})
#         self.client.force_authenticate(user=self.user)
#         response = self.client.get(url)

#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         self.assertEqual(response.data["message"], "Student class attendance retrieved successfully.")