...
from .basic import user_is_verified, user_is_staff, user_is_parent, user_is_student,
...
def user_is_staff_or_administrative(user):
    ...
    admin.
    """
    return user_is_admin_or_su(user) or user_is_staff(user) \
        if user.is_authenticated else False
...
def user_is_parent_or_administrative(user):
    ...
    admin.
    """
    return user_is_admin_or_su(user) or user_is_parent(user) \
        if user.is_authenticated else False
...
def user_is_student_or_administrative(user):
    ...
    admin.
    """
    return user_is_admin_or_su(user) or user_is_student(user) \
        if user.is_authenticated else False
...
def user_is_teacher_or_administrative(user):
    ...
    admin.
    """
    return user_is_admin_or_su(user) or user_is_teacher(user) \
        if user.is_authenticated else False
